...
class Model():
    ...
    def load_dataset(self,opt,eval_split="val"):
        ...
        log.info("loading training data [ scene {} ]...".format(opt.data.scene))
        ...
        log.info("loading test data [ scene {} ]...".format(opt.data.scene))
    ...
    def restore_checkpoint(self,opt):
        ...
        if opt.resume:
            log.warning("resuming from previous checkpoint...")
            epoch_start,iter_start = util.restore_checkpoint(opt,self,resume=opt.resume)
        elif opt.load is not None:
            log.warning("loading weights from checkpoint {}...".format(opt.load))
    ...
    def setup_visualizer(self,opt):
        log.info("setting up tensorboard...")
        if opt.tb:
    ...
    def train_iteration(self,opt,var,loader):
        ...
        self.it += 1
        loader.set_postfix(it=self.it,loss="{:.7f}".format(loss.all),psnr="{:.3f}".format(-10*loss.all.log10()))
    ...
    def summarize_acc_loss(self,opt,var,loss):
        loss_all = 0.
        assert("all" not in loss)
        # weigh losses
        for key in loss:
            ...
            if opt.loss_weight[key] is not None:
                assert not torch.isinf(loss[key]),"loss {} is Inf".format(key)
                assert not torch.isnan(loss[key]),"loss {} is NaN".format(key)
                loss_all += 10**float(opt.loss_weight[key])*(loss[key]/len(var.idx))
        loss.update(all=loss_all)
        return loss
    ...
    @torch.no_grad()
    def validate_training_state(self,opt,ep=None):
        self.graph.eval()
        for it, batch in enumerate(self.train_loader):
            var = edict(batch)
            var = util.move_to_device(var,opt.device)
            var = self.graph.forward(opt,var,mode="val")
            self.visualize_train(opt,var,step=ep,split="train")
            break
    ...
    @torch.no_grad()
    def visualize_train(self,opt,var,step=0,split="train"):
        raise NotImplementedError
...
class Graph(torch.nn.Module):
    ...
    def PSNR_loss(self,pred,label=0):
        loss = (pred.contiguous()-label)**2
        return 10*loss.mean().log10()
