...
from . import nerf_gaussian
import camera

# ============================ main engine for training and evaluation ============================

class Model(nerf_gaussian.Model):
    ...
    def generate_videos_pose(self,opt):
        ...
        for ep in range(0,opt.max_iter+1,opt.freq.ckpt):
            ...
            if opt.data.dataset in ["blender", "llff", "bleff"]:
...
class Graph(nerf_gaussian.Graph):

    def __init__(self,opt):
        super().__init__(opt)
        self.nerf = NeRF_Gaussian(opt)
        if opt.nerf.fine_sampling:
            self.nerf_fine = NeRF_Gaussian(opt)
...
class NeRF_Gaussian(nerf_gaussian.NeRF_Gaussian):
