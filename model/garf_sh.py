...
from . import nerf_gaussian_sh
import camera

# ============================ main engine for training and evaluation ============================

class Model(nerf_gaussian_sh.Model):

    def __init__(self,opt):
        super().__init__(opt)
        if not opt.init.pose:
            assert(opt.init.pose_warmup == 0), "Error: If not initialized with known pose, pose optimization has to start from iter 0"     
    ...
    def setup_optimizer(self,opt):
        ...
        if opt.init.pose:
            log.warning("initializing pose with given groundtruth")
        # set up scheduler
    ...
    def train_iteration(self,opt,var,loader):
        if self.it >= opt.init.pose_warmup:
    ...
    def log_scalars(self,opt,var,loss,metric=None,step=0,split="train"):
        ...
        # compute pose error
        if split=="train" and opt.data.dataset in ["blender","llff", "bleff"]:
            pose,pose_GT = self.get_all_training_poses(opt)
            pose_aligned,_ = self.prealign_cameras(opt,pose,pose_GT)
            error = self.evaluate_camera_alignment(opt,pose_aligned,pose_GT)
            self.tb.add_scalar("{0}/error_R_deg".format(split),torch.rad2deg(error.R.mean()),step)
    ...
    def evaluate_full(self,opt):
        ...
        error = self.evaluate_camera_alignment(opt,pose_aligned,pose_GT)
        log.info("evaluate pose accuracy...")
        print("----------------------------------------------------------")
        message = util.green("rotation (deg):", bold=True)
        message += "{:8.3f}\n".format(np.rad2deg(error.R.mean().cpu()))
        message += util.green("translation:", bold=True)
        message += "{:10.5f}".format(error.t.mean())   
        print(message)
        print("----------------------------------------------------------")
        # dump numbers
...
class Graph(nerf_gaussian_sh.Graph):
    ...
    def get_pose(self,opt,var,mode=None):
        if mode=="train":
            ...
            if opt.data.dataset=="blender":
                ...
                if opt.init.pose:
                    pose = camera.pose.compose([self.pose_eye, var.pose])
                else:
                    pose = self.pose_eye
            ...
            pose = camera.pose.compose([pose_refine,pose])
        elif mode in ["eval","test-optim", "val"]:
...
class NeRF_Gaussian_SH(nerf_gaussian_sh.NeRF_Gaussian_SH):
