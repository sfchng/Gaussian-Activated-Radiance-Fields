...
def plot_save_poses(opt,fig,pose,pose_ref=None,path=None,ep=None):
    ...
    plt.title("epoch {}".format(ep), fontsize=15)
    ...
    ax1.set_title("forward-facing view",pad=0, fontsize=15)
    ax2.set_title("top-down view",pad=0, fontsize=15)
    #plt.subplots_adjust(wspace=0,hspace=0)
...
def plot_save_poses_blender(opt,fig,pose,pose_ref=None,path=None,ep=None):
    ...
    for i in range(N):
        ...
        ax.scatter(cam_ref[i,5,0],cam_ref[i,5,1],cam_ref[i,5,2],color=ref_color,s=30)
    if ep==0:
